...
const getPlantingQuickActionNotes = async (
  plantingId: string,
): Promise<QuickActionNotesResponse> => {
  const { data } = await restClient.get(
    `/plantings/${plantingId}/quick-actions/notes`,
  );
  ...
  const items = rawItems
    ...
    .filter((item) => item.scope !== "bed")
    .sort((a, b) => (a.occurredAt ?? "").localeCompare(b.occurredAt ?? ""));

  return { items };
};

export const useGetPlantingQuickActionNotes = (plantingId: string | null) => {
  return useQuery({
    queryKey: plantingId
      ? quickActionKeys.plantingNotes(plantingId)
      : quickActionKeys.plantingNotes("unknown"),
    queryFn: () => getPlantingQuickActionNotes(plantingId as string),
